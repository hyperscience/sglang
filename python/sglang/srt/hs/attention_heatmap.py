...
OUTPUT_TOKEN_CHUNK_SIZE = 128  # Adjust this to balance VRAM vs. Latency
...
def compute_attn_weights(
    ...
    page_size: int,
    output_token_chunk_size: int = OUTPUT_TOKEN_CHUNK_SIZE,
) -> list[list[torch.Tensor]]:
    assert page_size == 1, "Implemented only for page_size == 1"
    assert len(key_cache_buffer) == len(query_buffer[0]), "Expecting same number of layers."
    ...
    for layer_id in range(num_layers):
        # 1. Prepare Keys [num_prompt_tokens, num_k_heads, head_dim]
        keys_base = key_cache_buffer[layer_id][prompt_token_indices, :, :]
        num_k_heads, head_dim = keys_base.shape[-2:]

        # 2. Reconstruct Queries [num_output_tokens, num_q_heads, head_dim]
        ...
        # 3. Handle GQA & Transpose Keys once
        ...
        layer_scores_cpu = torch.empty((num_output_tokens, num_prompt_tokens), dtype=torch.float16)

        # 4. Chunked Processing over output tokens
        # We don't loop over heads (fast), we loop over chunks of the output sequence
        for start_idx in range(0, num_output_tokens, output_token_chunk_size):
            end_idx = min(start_idx + output_token_chunk_size, num_output_tokens)
            
            # Extract chunk and reshape for BMM: [num_q_heads, chunk_len, head_dim]
            query_chunk = all_querys[start_idx:end_idx].transpose(0, 1).to(torch.float32)
            ...
            # Average over heads and offload
            # Result: [chunk_len, num_prompt_tokens]
        ...
        if layers_attn_weights_per_token is None:
            layers_attn_weights_per_token = [[token_attn] for token_attn in layer_scores_cpu]
        else:
